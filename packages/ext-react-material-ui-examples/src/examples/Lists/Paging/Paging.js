...
export default class PagingListExample extends Component {
    ...
    store = Ext.create("Ext.data.Store", {
        ...
        proxy: {
            type: "ajax",
            url: "/ExtReactMaterialUIExamples/Company",
            reader: {
                type: "json",
                rootProperty: "data",
            ...
            >
                <Container docked="top">
                    <div style={{ padding: "15px" }}>
                        ...
                        {bufferZone === false
                            ? "OFF"
                            : `ON (buffer zone: ${bufferZone})`}
                    </div>
                    <SegmentedButton>
                        <Button
                            text="Off"
                            pressed={bufferZone === false}
                            handler={() => this.setState({ bufferZone: false })}
                            flex={1}
                        />
                        <Button
                            text="0"
                            pressed={bufferZone === 0}
                            handler={() => this.setState({ bufferZone: 0 })}
                            flex={1}
                        />
                        <Button
                            text="8"
                            pressed={bufferZone === 8}
                            handler={() => this.setState({ bufferZone: 8 })}
                            flex={1}
                        />
                        <Button
                            text="16"
                            pressed={bufferZone === 16}
                            handler={() => this.setState({ bufferZone: 16 })}
                            flex={1}
                        />
                        <Button
                            text="32"
                            pressed={bufferZone === 32}
                            handler={() => this.setState({ bufferZone: 32 })}
                            flex={1}
                        />
                    </SegmentedButton>
                </Container>
